...
function tube_prep(data, color, init_step, name, max_pt) {
    ...
    var points = [];
    var dup = Math.ceil(max_pt / data.length);
    for (var i = 0, l = data.length; i < l; i++) {
        ...
        for (var n = 0, w = dup; n < w; n++) {
            out.rotation.push(new THREE.Euler(
                to_rad(data[i][4]),
                to_rad(data[i][3]),
                to_rad(data[i][5])));
    ...
    for (var i = 0, l=line_points.length-1; i < l; i++) {
        var dir = new THREE.Vector3(); // create once an reuse it
        var angle = dir.subVectors( line_points[i], line_points[i+1] ).normalize();
        var dist_pt = line_points[i].clone().addScaledVector(angle, 10);
        out.look_points.push(dist_pt);
    ...
    obj_loader.load(
        ...
        success = function (object) {
            ...
            out.object = object;
            out.object.position.set(line_points[out.pos_idx].x,
                    line_points[out.pos_idx].y, line_points[out.pos_idx].z);
            out.object.lookAt(
                out.look_points[out.pos_idx].x,
                out.look_points[out.pos_idx].y,
                out.look_points[out.pos_idx].z);

            out.object.setRotationFromEuler(out.rotation[out.pos_idx]);
